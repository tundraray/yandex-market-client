...
namespace Apix.Sync.YaMarket
{
    internal class UrlBuilder
    {
        ...
        private const string OffersWithLocationUrl = "models/{1}/offers?&page=1&count={2}&sort=PRICE&how=ASC&groupBy=SHOP&show_discounts=1&fields=OFFER_DISCOUNT%2COFFER_SHOP%2COFFER_DELIVERY%2CSHOP_RATING%2COFFER_VENDOR%2CVENDOR_ALL%2COFFER_PHOTO&uuid={0}&latitude={3}&longitude={4}";
        ...
        public string Offers(int id, string latitude, string longitude, int size = 5)
        {
            return string.Format("{0}{1}", apiUrl, string.Format(OffersWithLocationUrl, Guid.NewGuid().ToString("N"), id, size, latitude, longitude));
        }
